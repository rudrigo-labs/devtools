...
internal sealed class DependencyGraphBuilder
{
    ...
    public DependencyGraph Build(
        ...
        IReadOnlyList<string> ignoreUsingPrefixes,
        ...
        CancellationToken ct)
    {
        ...
        foreach (var file in files)
        {
            ...
            {
                if (!namespaceMap.TryGetValue(file.Namespace, out var nsList))
                {
                    nsList = new List<FileNode>();
                    namespaceMap[file.Namespace] = nsList;
                }
                nsList.Add(file);
            }
            ...
            {
                if (!typeMap.TryGetValue(typeName, out var typeList))
                {
                    typeList = new List<FileNode>();
                    typeMap[typeName] = typeList;
                }
                typeList.Add(file);
            }
        }

        var allowedExtSet = new HashSet<string>(allowedExtensions, StringComparer.OrdinalIgnoreCase);
        var fileByPath = files.ToDictionary(f => f.FullPath, StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var file in files)
        {
            ...
            if (file.Extension.Equals(".cs", StringComparison.OrdinalIgnoreCase))
            {
                foreach (var ns in ExtractUsings(file.Content, ignoreUsingPrefixes))
                {
                ...
                foreach (var identifier in ExtractTypeUsageTokens(file.Content))
                {
            ...
            else if (IsJsOrTs(file.Extension))
            {
                ...
                {
                    var target = ResolveJsTsImport(specifier, file.FullPath, rootPath, allowedExtSet, fileByPath);
    ...
    private static bool IsIgnoredUsing(string ns, IReadOnlyList<string> ignoredPrefixes)
    {
        foreach (var prefix in ignoredPrefixes)
        {
            ...
            if (ns.Equals(prefix, StringComparison.OrdinalIgnoreCase)) return true;
            if (ns.StartsWith(prefix + ".", StringComparison.OrdinalIgnoreCase)) return true;
        }
        return false;
    }
    ...
    private static FileNode? ResolveJsTsImport(
        ...
        IReadOnlyDictionary<string, FileNode> fileByPath)
    {
        if (string.IsNullOrWhiteSpace(specifier)) return null;

        var isRelative = specifier.StartsWith(".", StringComparison.Ordinal);
        var isRooted = specifier.StartsWith("/", StringComparison.Ordinal);

        if (!isRelative && !isRooted) return null;
        ...
        {
            if (fileByPath.TryGetValue(candidate, out var file))
                return file;
        }
        ...
        IReadOnlyDictionary<string, int> FanOut)
    {
        public int GetFanIn(string fullPath)
            => FanIn.TryGetValue(fullPath, out var v) ? v : 0;

        public int GetFanOut(string fullPath)
            => FanOut.TryGetValue(fullPath, out var v) ? v : 0;
    }
}
