...
public sealed class OrganizerEngine : IDevToolEngine<OrganizerRequest, OrganizerResult>
{
    private readonly IValidator<OrganizerRequest> _validator;
    private readonly TextExtractor _extractor = new();

    public OrganizerEngine(IValidator<OrganizerRequest>? validator = null)
    {
        _validator = validator ?? new OrganizerRequestValidator();
    }

    public Task<RunResult<OrganizerResult>> ExecuteAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!validation.IsValid)
        {
            var errs = validation.Errors
                .Select(e => new ErrorDetail($"organizer.{e.Field}", e.Message))
                .ToList();
            return Task.FromResult(RunResult<OrganizerResult>.Fail(errs));
        }

        if (!Directory.Exists(request.InboxPath))
        {
            return Task.FromResult(RunResult<OrganizerResult>.Fail(new ErrorDetail(
                "organizer.inbox.not_found",
                "Pasta de entrada não encontrada.",
                Cause: request.InboxPath)));
        }
        ...
        foreach (var file in files)
        {
            ...
            if (idx % 100 == 0)
                progress?.Report(new ProgressEvent(
                    $"Processados {idx}/{files.Count}",
                    (int)(idx * 100.0 / Math.Max(1, files.Count)),
                    "scan"));
            ...
            {
                plan.Add(new OrganizerPlanItem(file, UniqueTarget(duplicatesDir, fileName),
                    "DUPLICATE", "Nome igual (dedup por nome)", OrganizerAction.Duplicate));
                continue;
            }
            ...
            if (config.DeduplicateByHash)
            {
                ...
                {
                    plan.Add(new OrganizerPlanItem(file, UniqueTarget(duplicatesDir, fileName),
                        "DUPLICATE", "Hash igual (arquivo idêntico)", OrganizerAction.Duplicate));
                    continue;
                }
            }
            ...
            {
                plan.Add(new OrganizerPlanItem(file, UniqueTarget(duplicatesDir, fileName),
                    "DUPLICATE", "Primeiras linhas iguais (dedup por cabeçalho)", OrganizerAction.Duplicate));
                continue;
            }
        ...
        var stats = new OrganizerStats(
            plan.Count,
            eligibleCount,
            plan.Count(p => p.Action is OrganizerAction.WouldMove or OrganizerAction.Moved),
    ...
    private static void ScoreKeywords(
        ...
        ref int score)
    {
        if (keywords.Length == 0 || string.IsNullOrWhiteSpace(hay)) return;
    ...
    private static string BuildNameKey(string fileName)
    {
        ...
        return !string.IsNullOrWhiteSpace(normalized) ? normalized : TextNormalizer.Normalize(fileName);
    }
