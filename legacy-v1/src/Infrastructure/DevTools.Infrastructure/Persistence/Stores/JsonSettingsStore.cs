...
public sealed class JsonSettingsStore : ISettingsStore
{
    ...
    public bool IsConfigured()
    {
        ...
        {
            InfraLogger.Error($"Config file not found at '{_configPath}'.");
            return false;
        }
        ...
        try
        {
            ...
            {
                InfraLogger.Error($"Config file '{_configPath}' is empty.");
                return false;
            }
        ...
        {
            InfraLogger.Error($"Failed to read or parse config file '{_configPath}'.", ex);
            return false;
        }
    }
    ...
    public T GetSection<T>(string sectionName) where T : new()
    {
        if (!File.Exists(_configPath))
        {
            InfraLogger.Error($"Config file not found when reading section '{sectionName}'. Path: '{_configPath}'.");
            return new T();
        }
        ...
        try
        {
            ...
            InfraLogger.Error($"Config section '{sectionName}' not found in '{_configPath}'.");
        }
        catch (Exception ex)
        {
            InfraLogger.Error($"Failed to read config section '{sectionName}' from '{_configPath}'.", ex);
        }
    ...
    public void SaveSection<T>(string sectionName, T data)
    {
        ...
        {
            InfraLogger.Error($"Failed to save config section '{sectionName}' to '{_configPath}'.", ex);
        }
    }
    ...
    public void CreateDefaultIfNotExists()
    {
        ...
        {
            InfraLogger.Error($"Failed to create default config file at '{_configPath}'.", ex);
        }
    }
}
