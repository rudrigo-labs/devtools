...
public partial class App : System.Windows.Application
{
    ...
    private ToolConfigurationManager CreateToolConfigurationManager()
    {
        try
        {
            var configurationStore = ToolConfigurationStoreFactory.Create(_storageBackend);
            return new ToolConfigurationManager(configurationStore);
        }
        catch (Exception ex)
        {
            AppLogger.Error("Failed to initialize configuration store. Falling back to JSON configuration store.", ex);
