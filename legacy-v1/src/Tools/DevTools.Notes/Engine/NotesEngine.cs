...
public sealed class NotesEngine : IDevToolEngine<NotesRequest, NotesResponse>
{
    ...
    {
        _store = store;
        _itemsRepository = new NotesItemsRepository();
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            switch (request.Action)
            {
                ...
                case NotesAction.DeleteItem:
                    result = await HandleDeleteItemAsync(request, progress, ct).ConfigureAwait(false);
                    break;
    ...
    private static RunSummary BuildSummary(NotesRequest request, RunResult<NotesResponse> result, TimeSpan duration)
    {
        ...
        if (result.IsSuccess && result.Value is not null)
        {
            ...
            else if (val.DeleteResult != null)
            {
                processed = 1;
                changed = val.DeleteResult.Existed ? 1 : 0;
                output = val.DeleteResult.Path;
            }
        ...
        CancellationToken ct)
    {
        ...
        var result = await _itemsRepository.ListAsync(request.NotesRootPath, ct).ConfigureAwait(false);
        if (!result.IsSuccess || result.Value is null)
            return RunResult<NotesResponse>.Fail(result.Errors);
        ...
        CancellationToken ct)
    {
        if (ShouldUseSimplePathMode(request))
        {
            ...
            var simpleResult = await _itemsRepository.ReadAsync(request.NotesRootPath, request.NoteKey!, ct).ConfigureAwait(false);
    ...
    private async Task<RunResult<NotesResponse>> HandleDeleteItemAsync(
        ...
        CancellationToken ct)
    {
        progress?.Report(new ProgressEvent("Deleting note", 40, "delete"));

        var result = await _itemsRepository.DeleteAsync(request.NotesRootPath, request.NoteKey!, ct).ConfigureAwait(false);
        if (!result.IsSuccess || result.Value is null)
            return RunResult<NotesResponse>.Fail(result.Errors);

        progress?.Report(new ProgressEvent("Note deleted", 100, "done"));
        return RunResult<NotesResponse>.Success(new NotesResponse(
            request.Action,
            DeleteResult: result.Value));
    }
